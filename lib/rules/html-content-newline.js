...
function parseOptions (options) {
  return Object.assign({
    'singleline': 'ignore',
    'multiline': 'always',
    'hasChildElements': null,
    'ignoreNames': ['pre', 'textarea'],

    detectType (node) {
      if (this.hasChildElements && node.children.filter(c => c.type === 'VElement').length) {
  ...
  meta: {
    ...
    schema: [{
      ...
      properties: {
        ...
        'hasChildElements': { enum: ['ignore', 'always', 'never'] },
        'ignoreNames': {
          type: 'array',
          items: { type: 'string' },
          uniqueItems: true,
      ...
      'VElement' (node) {
        ...
        if (needBreaks !== hasBeforeBreaks) {
          context.report({
            ...
            fix: type === 'always'
              ...
              : (fixer) => fixer.removeRange([node.startTag.range[1], contentFirst.range[0]])
          })
        }
        ...
        if (needBreaks !== hasAfterBreaks) {
          context.report({
            ...
            message: 'Expected {{expected}} before open bracket of the "{{name}}" element, but {{actual}} found.',
            data: {
              name: node.name,
              expected: getBreakPhrase(needBreaks),
            ...
            fix: type === 'always'
              ...
              : (fixer) => fixer.removeRange([contentLast.range[1], node.endTag.range[0]])
          })
        }
      }
    })
